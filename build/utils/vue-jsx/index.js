...
var isInsideJsxExpression = function isInsideJsxExpression(t, path) {
  if (!path.parentPath) {
    return false;
  }
...
module.exports = function (babel) {
  ...
  return {
    ...
    visitor: {
      JSXNamespacedName: function JSXNamespacedName(path) {
        throw path.buildCodeFrameError('Namespaced tags/attributes are not supported. JSX is not XML.\n' + 'For attributes like xlink:href, use xlinkHref instead.');
      },
      JSXElement: {
      ...
      'Program': function Program(path) {
        path.traverse({
          'ObjectMethod|ClassMethod': function ObjectMethodClassMethod(path) {
            var params = path.get('params'); // do nothing if there is (h) param
            ...
            var isRender = path.node.key.name === 'render'; // inject h otherwise
          ...
          JSXOpeningElement: function JSXOpeningElement(path) {
            ...
            var typeAttribute = attributes.find(function (attributePath) {
              return attributePath.node.name && attributePath.node.name.name === 'type';
            });
  ...
  function buildOpeningElementAttributes(attribs, path) {
    ...
    if (objs.length === 1) {
      ...
      var helper = addDefault(path, 'babel-helper-vue-jsx-merge-props', {
