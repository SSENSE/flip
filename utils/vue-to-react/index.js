      ...
      if (name === "react-dom") {
        ...
        path.replaceWith(
          t.importDeclaration(
            ...
            t.stringLiteral("vue-styled-components")
          )
        );
      }
    }
  ...
  path.traverse({
    ...
    MemberExpression(path) {
     ...
     if (
        ...
        (object.get("property").node.name === "props" ||
          object.get("property").node.name === "$attrs") &&
        ...
        t.isIdentifier(property) &&
        property.node.name === "state"
      ) {
        property.replaceWith(t.identifier(dataIdentifier));
      } else if (
        t.isThisExpression(object) &&
        t.isIdentifier(property) &&
        property.node.name === "props"
      ) {
    ...
    JSXAttribute(path) {
      ...
      if (t.isJSXIdentifier(name) && name.node.name === "className") {
        path.replaceWith(
          t.jSXAttribute(t.jSXIdentifier("class"), path.get("value").node)
        );
      }
    },
...
const convertReactComponent = (t, path, isDefaultExport) => {
    ...
    BlockStatement(path) {
      ...
      path.traverse({
        CallExpression(path) {
          ...
          if (
            ...
            eventMap[callee.node.name] === undefined
          ) {
            return;
          }
          callee.replaceWith(
      ...
      path.traverse({
        CallExpression(path) {
          ...
          const eventNameUppercased = callee
            .get("property")
            .node.name.replace(eventRE, "");
          const eventName = `${eventNameUppercased[0].toLowerCase()}${eventNameUppercased.slice(
            1
          )}`;
          ...
          path.replaceWith(
            t.callExpression(
              ...
              [t.stringLiteral(eventName), ...args]
            )
          );
        }
      });
  ...
  reactBody.get("body").forEach(reactProperty => {
    ...
    // proptypes
    if (key.node.name === 'propTypes' ) {
        vueBody.push(
            t.objectProperty(
                t.identifier("props"),
                t.arrowFunctionExpression([], () => reactProperty.get("value").node)
            ))
    }
    ...
    if (
      ...
      (reactProperty.node.kind === "method" ||
        reactProperty.node.kind === "constructor") &&
      t.isIdentifier(key)
    ) {
      ...
      if (reactProperty.node.kind === "constructor") {
        ...
        body.node.body.push(
          t.returnStatement(
            t.memberExpression(t.thisExpression(), t.identifier("__state"))
          )
        );
      ...
      if (!t.isBlockStatement(body)) {
        arrowFn
          .get("body")
          .replaceWith(t.blockStatement([t.returnStatement(body)]));
  ...
  if (methods.length > 0) {
    vueBody.push(
      t.objectProperty(t.identifier("methods"), t.objectExpression(methods))
    );
  }
  ...
  if (isDefaultExport) {
    ...
    path.replaceWith(
      t.variableDeclaration("const", [
        t.variableDeclarator(id.node, t.objectExpression(vueBody))
      ])
    );
  }
};
...
module.exports = ({ types: t }) => {
  return {
    visitor: {
      Program(path) {
        const componentIdentifier = getImportIdentifier(
          t,
          path,
          "react",
          "Component"
        );
        const renderIdentifier = getImportIdentifier(
          t,
          path,
          "react-dom",
          "render"
          ...
          ClassDeclaration(path) {
            ...
            if (superClass) {
              const isReactDotComponent =
                ...
                superClass.node.object.name === "React" &&
                superClass.node.property.name === componentIdentifier;

              const isComponent =
                t.isIdentifier(superClass) &&
                superClass.node.name === componentIdentifier;
